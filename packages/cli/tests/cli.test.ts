...
describe('runCli', () => {
  ...
  describe('successful scaffold', () => {
    ...
    it('materializes the Go + Flutter mobile composition when answers are provided (issue #19)', async () => {
      const result = await runCli(['go-mobile-app'], {
        noExit: true,
        answers: {
          backend: 'go',
          topology: 'monolith',
      ...
      const pub = await readFile(join(workDir, 'go-mobile-app', 'apps/mobile/pubspec.yaml'), 'utf8');
      expect(pub).toContain('flutter_secure_storage');
      const dartClient = await readFile(
        join(workDir, 'go-mobile-app', 'packages/contract/clients/dart/lib/openapi_client.dart'),
        'utf8',
      );
